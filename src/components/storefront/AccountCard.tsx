...
export function AccountCard({ account }: { account: PublicAccount }) {
  ...
  return (
    // overflow-hidden on the Link itself: clips children (prevents mobile overflow)
    // but does NOT clip the Link's own box-shadow → shadow visible ✅
    <Link
    ...
    >
      <div className="flex h-full w-full flex-col rounded-2xl border border-slate-200 bg-white transition-colors duration-300 hover:border-indigo-200">
        {/* Image + overlay CTA */}
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            ...
            <div className="flex min-w-0 items-center gap-1.5 text-sm text-slate-500">
              <Coins className="h-3.5 w-3.5 shrink-0 text-yellow-500" />
              <span className="truncate">
